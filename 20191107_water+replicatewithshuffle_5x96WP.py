...
Source 96WPs in 2
Destination 96WPs in 1, 4, 7, 10,11
...
drugs_mapping = {}
seed = 20191107 # for reproducibility. Let's use the experimental date for the actual experiment, something else for debugging
np.random.seed(seed)
src_cols = np.arange(n_columns) # array of column numbers
# bug here, changing code
# only keep one source, many destinations
# code as before only worked for slots with one digit
...
for i,(k,v) in enumerate(drugs_mapping.items()):
    print(i)
    # unpack slots
