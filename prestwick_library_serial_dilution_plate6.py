...
drug_wells = [row + col for row in 'ABCDEFGH' for col in high_conc_cols]
# list comprehension is not very effective way of doing list(set-set),
# but it's small lists AND ORDER MATTERS!!
drug_wells = [w for w in drug_wells if w not in (DMSO_wells + H2O_wells)]
assert len(drug_wells) == 8+7*3
...
high_conc_drugs_wells = lib_plate.wells(*[drug_wells])
...
for well in high_conc_drugs_wells:
    serially_dilute_well(well)

# tips used:
# 2 tips for controls
# 1 tip for each drug to put solvent => 29 tips
# 2 tips per drug to dilute => 58 tips
# total is 89 tips (58 10ul tips and 31 200ul tips)
count_used_tips()
